...
namespace BzKovSoft.ObjectSlicer.Polygon
{
	...
	public class BzPoly
	{
		...
		private LoopIndex[] CombineLoops(BzPolyLoop outer, BzPolyLoop[] inners)
		{
			...
			var innersData = new LinkedLoop<LoopIndex>[inners.Length];
			...
			var result = new LoopIndex[outerData.size];
			var node = outerData.first;
		...
		private bool GetShortestConnection(LinkedLoop<LoopIndex> outerData, LinkedLoop<LoopIndex> innerData, BzPolyLoop[] allLoops, out LoopNode<LoopIndex> minOliNode, out LoopNode<LoopIndex> minIliNode)
		{
			minOliNode = null;
			minIliNode = null;
			float curDist = float.MaxValue;

			var oliNode = outerData.first;
			for (int oli = 0; oli < outerData.size; oli++)
			{
				var oliData = oliNode.value;
				var iliNode = innerData.first;
				for (int ili = 0; ili < innerData.size; ili++)
				{
		...
		private static LinkedLoop<LoopIndex> GetIndexCollection(BzPolyLoop loop)
		{
		...
		public PolyMeshData GetMeshData()
		{
			...
			for (int i = 0; i < meshData.triangles.Length; i += 3)
			{
				...
				var normal = BzSlicerHelper.Normalize(Vector3.Cross(BzSlicerHelper.Normalize(dir1), BzSlicerHelper.Normalize(dir2)));
			...
			{
				var n = BzSlicerHelper.Normalize(meshData.normals[i]);
		...
		private void OptimizeData(out LoopIndex[] newLoopIndexers, out int[] triangles)
		{
			...
			for (int i = 0; i < length; i++)
			{
				...
				for (int l = i + 1; l < length; l++)
				{
					...
					if (vertexIndex == vertexIndex2 & iindex != iindex2)
					{
