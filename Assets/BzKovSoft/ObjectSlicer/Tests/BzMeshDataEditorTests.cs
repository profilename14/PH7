...
namespace BzKovSoft.ObjectSlicer.Tests
{
	public class BzMeshDataEditorTests
	{
		...
		public void GetEdgeLoops()
		{
			...
			var arr = LoopToArray(loop);
			if (arr[1] == 2) arr[1] = 1;  // vertex 1 and 2 are the same, so it does not matter which one to remove

			CollectionAssert.AreEqual(new[] { 0, 1, 3, 4 }, arr);
		}
